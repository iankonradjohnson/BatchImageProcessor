...
class ScanTailorService:
    ...
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize the ScanTailor service with configuration.
        
        Args:
            config: Configuration for ScanTailor operations
        """
        self.enabled = config.get('enabled', True)
        self.book_type = config.get('book_type', 'standard')
        self.color_mode = config.get('color_mode')
        self.force_color = config.get('force_color', False)
        self.dpi = config.get('dpi', 600)
        self.compression = config.get('compression', 'lzw')
    ...
    def process_images(self, input_dir: str, output_dir: str) -> bool:
        ...
        # Run ScanTailor
        logger.info(f"Running ScanTailor command: {' '.join(cmd)}")
        try:
    ...
    def _run_fallback_scantailor(self, input_dir: str, output_dir: str, color_mode: str) -> bool:
        ...
        logger.info(f"Running fallback ScanTailor command: {' '.join(cmd)}")
        try:
