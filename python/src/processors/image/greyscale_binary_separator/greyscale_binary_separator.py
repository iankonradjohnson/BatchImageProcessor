...
class GreyscaleBinarySeparator(ImageProcessor):
    ...
    def _generate_detection_report(self, image: np.ndarray, regions: List[Region], probability_map: np.ndarray) -> None:
        """
        ...
        report = {
            ...
            "configuration": {
                "region_extraction": self.config.get('detection', {}).get('region_extraction', {}),
                "strategy_weights": self.config.get('detection', {}).get('strategy_weights', {})
            }
        }
        ...
        print(f"Grayscale regions: {grayscale_regions_count} regions, {grayscale_percentage:.2f}% of image")
        print(f"Binary regions: {binary_regions_count} regions, {binary_percentage:.2f}% of image")
    ...
    def _save_diagnostic_visualizations(
        ...
        threshold = self.config.get('detection', {}).get('region_extraction', {}).get('threshold', 0.5)
        plt.axvline(x=threshold, color='r', linestyle='--', label=f'Threshold ({threshold})')
        plt.legend()
        
        # Save histogram
        hist_path = os.path.join(diagnostics_dir, f'probability_histogram_{timestamp}.png')
        ...
        # Save heatmap
        heatmap_path = os.path.join(diagnostics_dir, f'probability_heatmap_{timestamp}.png')
        ...
        if region_sizes:
            ...
            grayscale_sizes = [size for size, rtype in zip(region_sizes, region_types) if rtype == 'grayscale']
            binary_sizes = [size for size, rtype in zip(region_sizes, region_types) if rtype == 'binary']
