...
class RegionDetectionEngine:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.strategy_weights = self.config.get(
            "strategy_weights", {"histogram": 0.4, "texture": 0.4, "edge": 0.2}
        )
    ...
    def detect_regions(self, image: np.ndarray) -> Tuple[List[Region], np.ndarray]:
        ...
        print(
            f"Identified approximately {np.sum(text_line_mask) / (h * w) * 100:.2f}% of the image as text/line features"
        )
        ...
        for y in range(0, h - window_size + 1, stride):
            for x in range(0, w - window_size + 1, stride):
                ...
                window_text_mask = text_line_mask[
                    y : y + window_size, x : x + window_size
                ]
                text_density = (
                    np.mean(window_text_mask) if window_text_mask.size > 0 else 0
                )
        ...
        print(
            f"Global image metrics - StdDev: {global_std:.2f}, Unique Values: {len(np.unique(gray_img))} ({global_unique:.2%})"
        )
        ...
        if global_std > 80 and global_unique > 0.6:
            print(
                "IMAGE APPEARS TO BE MOSTLY PHOTOGRAPHIC - Forcing grayscale detection"
            )
        ...
        print(
            f"Enhanced detection - Min: {np.min(probability_map):.2f}, Max: {np.max(probability_map):.2f}, Mean: {np.mean(probability_map):.2f}"
        )
        print(
            f"Non-zero probability pixels: {np.sum(probability_map > 0) / (h * w) * 100:.2f}%"
        )
        ...
        if grayscale_regions:
            binary_region = self.region_extractor.invert_regions(
                grayscale_regions, image.shape[:2]
            )
    ...
    def visualize_regions(
        ...
        if probability_map is not None:
            ...
            cmap = LinearSegmentedColormap.from_list(
                "my_cmap", ["blue", "lightblue", "yellow", "red"]
            )
