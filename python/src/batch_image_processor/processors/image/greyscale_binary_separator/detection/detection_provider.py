...
class DetectionStrategyProvider:
    ...
    def register_strategy(
        self, strategy_name: str, strategy_class: Type[DetectionStrategy]
    ) -> None:
        """
    ...
    def get_strategy(
        self, strategy_name: str, config: Dict[str, Any] = None
    ) -> DetectionStrategy:
        """
        ...
        if strategy_name not in self._strategies or config is not None:
            self._strategies[strategy_name] = self._strategy_classes[strategy_name](
                config
            )
    ...
    def combine_results(
        self, results: List[np.ndarray], weights: List[float] = None
    ) -> np.ndarray:
        """
        ...
        combined_result = np.sum(
            stacked_results * weights[:, np.newaxis, np.newaxis], axis=0
        )
