...
class TextureAnalysisStrategy(BaseDetectionStrategy):
        ...
        try:
            ...
            for i in range(windows_lbp.shape[0]):
                for j in range(windows_lbp.shape[1]):
                    ...
                    texture_measure = self._calculate_texture_measure(
                        window_lbp, window_gray
                    )
                    ...
                    grayscale_prob = max(
                        0.3, min(1.0, texture_measure / (self.texture_threshold * 0.5))
                    )
    ...
    def _compute_lbp(self, image: np.ndarray) -> np.ndarray:
        ...
        lbp = local_binary_pattern(
            image, self.lbp_points, self.lbp_radius, method="uniform"
        )
        return lbp

    def _calculate_texture_measure(
        self, lbp_window: np.ndarray, gray_window: np.ndarray
    ) -> float:
        """
        ...
        texture_measure = (
            0.5 * entropy / np.log2(max_lbp_val) + 0.5 * normalized_variance
        )
