...
class RegionExtractor:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.text_perimeter_area_threshold = text_detection.get(
            "text_perimeter_area_threshold", 0.08
        )
        self.min_text_circularity = text_detection.get("min_text_circularity", 0.1)

        # Size-based shape requirements
        self.very_large_region_multiplier = self.config.get(
            "very_large_region_multiplier", 10
        )
        self.large_region_ratio_multiplier = self.config.get(
            "large_region_ratio_multiplier", 1.2
        )
        self.large_region_circularity_multiplier = self.config.get(
            "large_region_circularity_multiplier", 0.6
        )

        self.medium_region_divider = self.config.get("medium_region_divider", 4)
        self.medium_region_ratio_multiplier = self.config.get(
            "medium_region_ratio_multiplier", 0.9
        )
        self.medium_region_circularity_multiplier = self.config.get(
            "medium_region_circularity_multiplier", 0.8
        )

        self.small_region_ratio_multiplier = self.config.get(
            "small_region_ratio_multiplier", 0.5
        )
        self.small_region_circularity_multiplier = self.config.get(
            "small_region_circularity_multiplier", 2.0
        )
    ...
    def extract_regions(self, probability_map: np.ndarray) -> List[Region]:
        ...
        print(
            f"Probability map stats - Min: {np.min(probability_map):.4f}, Max: {np.max(probability_map):.4f}, Mean: {np.mean(probability_map):.4f}"
        )
        ...
        if np.max(probability_map) < 0.5:
            ...
            print(
                f"Enhanced map stats - Min: {np.min(probability_map):.4f}, Max: {np.max(probability_map):.4f}, Mean: {np.mean(probability_map):.4f}"
            )

        # Only fall back to synthetic mask if absolutely necessary
        if np.max(probability_map) == 0:
            print(
                "WARNING: Probability map is all zeros. Creating synthetic grayscale regions..."
            )
        ...
        for prop in props:
            ...
            if area > very_large_area:
                ...
                if looks_like_text and circularity < self.min_text_circularity:
                    is_blob = False
                    print(
                        f"Rejecting large text-like region with area {area}, P/A ratio {perimeter_area_ratio:.4f}"
                    )
                else:
            ...
            # For large regions, use relaxed requirements
                ...
                is_blob = (
                    ...
                    <= self.max_perimeter_area_ratio
                    * self.large_region_ratio_multiplier
                    and circularity
                    >= self.blob_circularity * self.large_region_circularity_multiplier
                    and not looks_like_text
                )  # Explicit text check
            ...
            # For medium regions, use standard requirements
                ...
                is_blob = (
                    perimeter_area_ratio
                    <= self.max_perimeter_area_ratio
                    * self.medium_region_ratio_multiplier
                    and circularity
                    >= self.blob_circularity * self.medium_region_circularity_multiplier
                    and not looks_like_text
                )
            ...
            # For small regions, be very strict to avoid text and lines
                ...
                is_blob = (
                    perimeter_area_ratio
                    <= self.max_perimeter_area_ratio
                    * self.small_region_ratio_multiplier
                    and circularity
                    >= self.blob_circularity * self.small_region_circularity_multiplier
                    and area >= self.small_region_min_area
        ...
        if perimeter_area_ratios:
            print(
                f"Shape analysis - Perimeter/Area ratios: min={min(perimeter_area_ratios):.4f}, "
                f"max={max(perimeter_area_ratios):.4f}, mean={np.mean(perimeter_area_ratios):.4f}"
            )
            print(
                f"Shape analysis - Circularities: min={min(circularities):.4f}, "
                f"max={max(circularities):.4f}, mean={np.mean(circularities):.4f}"
            )
            print(
                f"Shape analysis - Areas: min={min(areas)}, max={max(areas)}, mean={np.mean(areas):.1f}"
            )
        ...
        if len(regions) > 20:
            print(
                f"Found {len(regions)} regions after filtering, still merging nearby ones..."
            )
        ...
        if len(regions) > 0:
            ...
            print(
                f"Grayscale pixels: {grayscale_pixels} ({grayscale_pixels/total_pixels*100:.2f}% of image)"
            )

        return regions

    def _merge_nearby_regions(
        self, regions: List[Region], distance_threshold: int = 30
    ) -> List[Region]:
        """
        ...
        while True:
            ...
            for i in range(len(regions) - 1):
                ...
                for j in range(i + 1, len(regions)):
                    ...
                    if x_distance > 0:
                        distance = (
                            max(0, y_min_j - y_max_i)
                            if y_min_j > y_max_i
                            else max(0, y_min_i - y_max_j)
                        )
                    elif y_distance > 0:
                        distance = (
                            max(0, x_min_j - x_max_i)
                            if x_min_j > x_max_i
                            else max(0, x_min_i - x_max_j)
                        )
                    else:
                        # No overlap in either dimension, use Manhattan distance
                        distance = min(
                            abs(x_min_i - x_max_j), abs(x_min_j - x_max_i)
                    ...
                    if distance < distance_threshold:
                        ...
                        avg_confidence = (
                            regions[i].confidence + regions[j].confidence
                        ) / 2
                        # Create new region
    ...
    def create_region_mask(
        self, regions: List[Region], shape: Tuple[int, int]
    ) -> np.ndarray:
        """
