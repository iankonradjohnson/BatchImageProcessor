...
class TestDualPageCropper(TestCase):
    ...
    def test_process(self):
        ...
        self.assertEqual(cropped_left.getpixel((40, 100)), (0, 0, 255))  # Check for blue pixel in the middle

        # Test right crop
        cropped_right = cropper.process(self.img)
        self.assertEqual(cropped_right.getpixel((60, 100)), (255, 0, 0))  # Check for red pixel in the middle

        # Test toggling back to left
        cropped_left_again = cropper.process(self.img)
        self.assertEqual(cropped_left_again.getpixel((40, 100)), (0, 0, 255))  # Check for blue pixel again
